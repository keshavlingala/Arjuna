...
// TODO To be examined RED Alert
public class Program1 {
    static ArrayList<ArrayList<Integer>> graph = new ArrayList<>();

    public static int get_days(int src, boolean vis[], int parent[]) {
        for (int x : graph.get(src)) {
            if (!vis[x]) {
                vis[x] = true;
                if (parent[x] == -1 || get_days(parent[x], vis, parent) != 0) {
                    parent[x] = src;
                    return 1;
                }
            }
